...
export const getRootOperationId = ({ operations }) => {
  const nullParentOperations = Object.values(operations.entities).filter(
...
export const getOperationByTableId = (state, tableId) => {
  const operation = Object.values(state.operations.entities).find((operation) =>
    operation.children.map(({ id }) => id).includes(tableId)
  );
  return operation;
};
