...
OpenRoundup is a web-based data wrangling tool that enables users to combine tabular data through visual operations. The system leverages **DuckDB-WASM** for client-side SQL execution, **Redux Saga** for complex async workflows, and **React** with **Material-UI** for the user interface. It provides the following key features:
...
- `npm run dev:docs`: Start the Jekyll server for documentation
- `npm run build`: Build both the application and documentation
- `npm run build:demo`: Build the application for production
- `npm run build:docs`: Build the Jekyll documentation
- `npm run preview`: Serve the built application locally, requires `npm run build` first
- `npm run preview:docs`: Serve the built documentation locally, requires `npm run build:docs` first
- `npm run clean`: Removes all files in the `build` directory
- `npm run lint`: Run ESLint to check for code quality issues
- `npm run test`: Run Jest tests with coverage reporting
- `npm run test:cy`: Run Cypress end-to-end tests
- `npm run test:cy:open`: Open Cypress Test Runner for interactive testing
- `npm run project-stats`: Generate codebase statistics
- `npm run deploy`: Deploy the application to demo server.
# roundup
