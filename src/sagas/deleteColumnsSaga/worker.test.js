...
describe("deleteColumnsWorker saga", () => {
  ...
  describe("failed column deletion", () => {
    it("dispatches failure action when database drop fails", async () => {
      // Mock dropColumns to reject
      dropColumns.mockRejectedValue(new Error("Drop failed"));
      ...
      const failureAction = effects.put.find(
        (effect) => effect.payload.action.type === deleteColumnsFailure.type
      );
      expect(failureAction).toBeDefined();
      expect(failureAction.payload.action.payload.columnIds).toContain("c_1");
    });
  });
