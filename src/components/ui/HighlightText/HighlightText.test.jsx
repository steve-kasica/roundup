...
describe('HighlightText', () => {
  it('renders text without highlighting when there is no match', () => {
    ...
    expect(screen.queryByText(/.*/, { selector: 'span.highlight' })).not.toBeInTheDocument();
  });
  ...
  it('highlights the matching text when there is a match', () => {
    ...
    expect(screen.getByText('world', { selector: 'span.highlight' })).toBeInTheDocument();
  });
  ...
  it('matches only the first occurrence when multiple matches exist', () => {
    const { container } = render(<HighlightText text="Hello world, welcome to the world" pattern="world" />);
  ...
  it('handles non-string text inputs by converting to string', () => {
    ...
    expect(screen.getByText('4', { selector: 'span.highlight' })).toBeInTheDocument();
    expect(screen.getByText('2')).toBeInTheDocument();
  });

  it('preserves original casing in highlighted text', () => {
    const { container } = render(<HighlightText text="Hello WORLD" pattern="world" />);
