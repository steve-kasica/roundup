...
public class TestProjectJoinOperation extends RoundupTest {
    ...
    public class TestRows {
        ...
        public void testCellSize() {
            ...
            int expected = interviews.rows.get(0).cells.size() + employees.rows.get(0).cells.size() - 1;
    ...
    public class TestColumns {
        ...
        public void setup() {
            ...
            results = ProjectJoinOperation.joinColumns(
                    interviews.columnModel.columns,
                    employees.columnModel.columns);

        }

        @Test
        public void testColumnCount() {
            int expected = interviews.columnModel.columns.size() + employees.columnModel.columns.size();
            System.out.println(interviews.columnModel.columns);
            int actual = results.size();
            Assertions.assertEquals(expected, actual);
        }

        @Test
        @DisplayName("Column count of input projects is unchanged")
        public void testInputsColumnCount() {
            int expected, actual;

            expected = originalInterviewsCount;
            actual = interviews.columnModel.columns.size();
            Assertions.assertEquals(expected, actual);

            expected = originalEmployeesCount;
            actual = employees.columnModel.columns.size();
            Assertions.assertEquals(expected, actual);
        }
        ...
        public void testColumnIdentity() {
            assert results.size() > 0;
            List<Column> combined = Stream.concat(interviews.columnModel.columns.stream(), employees.columnModel.columns.stream())
